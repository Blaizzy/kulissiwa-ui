...
<div
    ...
    :class="{ 'opacity-0 pointer-events-none': !showModal }">
        <div class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div class="bg-white rounded-lg shadow-lg w-1/3 z-50 flex flex-col text-gray-200 dark:bg-neutral-900">
        ...
        <div class="w-1/2 parent-card space-x-2 bg-white p-2 px-4 dark:bg-inherit">
          <div class="flex items-center justify-between py-2 px-2">
            ...
            <h3 class="animated-gradient-text text-lg"> Upload Documents </h3>
            Ask anything about PDFs, Docx, text or urls and get answers in seconds.
          </div>
      ...
      <div class="flex items-center justify-center space-x-4 mx-4 bg-white rounded-lg dark:bg-neutral-900 pb-4">
        <div v-show="isLoading" class="flex space-x-4 bg-white px-2 py-10 dark:bg-neutral-900">
          <LoadingIndicatorModal />
        </div>
        
        <div class="w-1/2 parent-card space-x-2 bg-white p-2 px-4 rounded-lg border-2 border-gray-200 dark:bg-neutral-800 dark:border-none"
          v-for="price in prices" :key="price.id">
  
            <div class="flex items-center justify-between py-2 px-2"
            >
              <h3 
              class="animated-gradient-text text-lg" v-if="price.recurring.interval == 'month'"
              > Monthly </h3>
              <h3 class="animated-gradient-text text-lg" v-else
              > Yearly </h3>
            ...
            <p class="text-md font-regular text-gray-400"
            :class="{ 'mb-10': price.recurring.interval == 'month', 'mb-4': price.recurring.interval != 'month'}"
            > billed per {{ price.recurring.interval }} </p>
            
            <p class="text-md font-regular animated-gradient-text " v-if="price.recurring.interval != 'month'"> <ClientOnly><i class="fas fa-check text-sky-600 pr-1"></i></ClientOnly> 14 day free trial </p>
